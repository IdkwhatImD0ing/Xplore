...
export default function Wizard() {
  ...
  const [citiesObjects, setCitiesObjects] =
      useState([]); // [ {name: 'city', lat: 0, lng: 0}, ... ]
  ...
  const submitAttractions = () => {
    ...
    const places = [...citiesObjects, ...selectedAttractions ].map(
        (place) => ({
          name : place.name,
          lat : place.lat,
          lon :
              place.lng ||
                  place.lon, // Use lng or lon depending on the object structure
        }));
    ...
    fetch("http://localhost:8000/generate-route/", {
      ...
      body : JSON.stringify(payload),
        ...
        .then((routeData) => {
          ...
          const updatedRoutes = routeData.map(
              (dayRoutes) => dayRoutes.map(
                  (route) => route.map((placeName) => {
                    const placeObject =
                        places.find((p) => p.name === placeName);
  ...
  return (
    <Cities
  cities = {cities} setCities = {setCities} preferences =
      {preferences} setPreferences = {
          setPreferences} continueFunction = { citiesSubmit } />
  );
}
