...
>
  {['Discover Hidden Treasures', 'Plan Itineraries Effortless', 'Navigate Seamlessly Real-Time'].map((title, index) => (
    <Box
      key={index}
    ...
    >
      <Heading fontSize="4xl" mb={6} textAlign="center" fontWeight="bold" color = "purple.900">
        {title}
      </Heading>
      <Text fontSize="xl" mb={4} color = "purple.900">
        {index === 0 && 'Xplore revolutionizes travel by guiding you to hidden gems and local favorites. Experience unique destinations like never before with personalized recommendations tailored to your interests and travel style.'}
        {index === 1 && "Say goodbye to travel planning headaches! Xplore's intuitive interface simplifies itinerary creation, allowing you to seamlessly plan your journey, from accommodations to activities, all in one convenient app."}
        {index === 2 && "Stay informed and navigate effortlessly with Xplore's real-time GPS and location-based insights. Whether you're seeking the best nearby dining spots or looking for the shortest route to your next adventure, Xplore has you covered."}
      </Text>
    </Box>
  ))}
</Flex>
